...
export default function VehicleBodyInspection({ parts, value, onChange }: Props) {
  ...
  function updateInspection(partCode: string, changes: Partial<Inspection>) {
    const existing = inspectionMap.get(partCode);
    const next: Inspection = {
      part_code: partCode,
      condition: existing?.condition || "intact",
      paint_thickness_microns: existing?.paint_thickness_microns ?? null,
      notes: existing?.notes ?? null,
      ...changes,
    };

    const filtered = value.filter((item) => item.part_code !== partCode);
    if (
    ...
    <section dir="rtl" className="rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="p-5 sm:p-6">
        ...
        <VehicleBodyMap
          ...
          value={value}
          onPartClick={(code) =>
            setSelectedPart(activeParts.find((part) => part.code === code) ?? null)
          }
        />

        {selectedPart && (
          <div
            className="fixed inset-0 z-[100] flex h-[100dvh] w-full flex-col bg-slate-950/50 p-0 backdrop-blur-sm"
            role="dialog"
            aria-modal="true"
          >
            <div className="mx-auto flex h-full w-full max-w-lg flex-col bg-white shadow-2xl">
              <div className="shrink-0 border-b border-slate-200 bg-white px-5 py-4">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-lg font-extrabold text-slate-950">{selectedPart.name_fa}</div>
                    <div className="mt-1 text-xs text-slate-400">ثبت نتیجه کارشناسی</div>
                  </div>
                  <button
                    type="button"
              ...
              <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-5 pb-6">
                <div className="mb-3 text-sm font-bold text-slate-700">وضعیت قطعه</div>
                <div className="grid grid-cols-2 gap-2">
                  {CONDITIONS.map((condition) => {
                    const active = getPartStatus(selectedPart.code) === condition.value;
                    return (
                      <button
                        key={condition.value}
                        type="button"
                        onClick={() => updateInspection(selectedPart.code, { condition: condition.value })}
