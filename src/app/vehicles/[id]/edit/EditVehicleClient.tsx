...
import { deleteVehicleImageAction } from "./imageMutations";
...
export default function EditVehicleClient({ initialData }: { initialData: VehicleEditPageData }) {
  ...
  const [vehicleBrands] = useState<VehicleBrand[]>(initialData.brands);
  ...
  async function handleCatalogSearchSelect(selection: VehicleCatalogSelection) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "انتخاب خودرو از کاتالوگ ناموفق بود.");
    }
  }

  async function checkVehicleDuplicates() {
    if (!vehicle?.dealership_id || !brand.trim() || !model.trim()) {
  ...
  async function deleteExistingImage(image: VehicleImage) {
    ...
    if (storageError) {
      setError(`حذف فایل تصویر ناموفق بود: ${storageError.message}`);
      setDeletingImageId(null);
      return;
    }
    const result = await deleteVehicleImageAction(image.id, id);
    if (!result.ok) {
      setError(`حذف اطلاعات تصویر ناموفق بود: ${result.error}`);
      setDeletingImageId(null);
      return;
    }
  ...
  async function updateVehicle(e: React.FormEvent) {
    ...
    const vehicleResult = await updateVehicleAction({
      vehicleId: id,
      brand,
      model,
      trim: trim.trim() || null,
      modelYear: yearValue ?? 0,
    ...
    if (newImages.length > 0) {
      ...
      try {
        ...
        const added: VehicleImage[] = imageRows.map((row, index) => ({ id: `new-${Date.now()}-${index}`, storage_path: row.storagePath, thumbnail_path: row.thumbnailPath, sort_order: row.sortOrder }));
        setImages((current) => [...current, ...added].sort((a, b) => a.sort_order - b.sort_order));
        setNewImages([]);
      } catch (imageError) {
  ...
  if (!vehicle) {
    return <AdminLayout title="ویرایش خودرو" description="ویرایش اطلاعات خودرو"><div className="rounded-2xl bg-white p-8 shadow-sm">خودرو پیدا نشد.</div></AdminLayout>;
  }
        ...
        <form onSubmit={updateVehicle} className="space-y-6">
          ...
          {!duplicateChecking && duplicateVehicles.length > 0 && (
            <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5">
              <h3 className="font-extrabold text-amber-950">خودروی بسیار مشابه پیدا شد</h3>
              <p className="mt-1 text-sm text-amber-800">یک یا چند خودرو با مشخصات مشابه قبلاً در این نمایشگاه ثبت شده‌اند.</p>
              <div className="mt-4 space-y-2">{duplicateVehicles.map((item) => <div key={item.vehicle_id} className="rounded-xl border border-amber-200 bg-white p-3"><div className="font-bold">{item.brand} {item.model}{item.trim_name ? ` · ${item.trim_name}` : ""}</div><div className="mt-2 text-xs text-slate-500">تطابق {item.similarity_score}٪</div></div>)}</div>
            </div>
          )}

          <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <h2 className="mb-6 text-xl font-bold">اطلاعات خودرو</h2>
            <VehicleCatalogSearch label="جستجوی سریع خودرو" placeholder="مثلاً پژو 207 MC، Toyota Corolla..." initialQuery={[brand, model, trim].filter(Boolean).join(" ")} onSelect={handleCatalogSearchSelect} initialCatalog={{ brands: vehicleBrands, models: vehicleModels, trims: vehicleTrims }} />
            <div className="mt-5 grid gap-4 md:grid-cols-2">
              <label className="text-sm text-gray-600">برند *<select required value={brandId} onChange={(e) => handleBrandChange(e.target.value)} className="mt-2 w-full rounded-xl border border-gray-200 bg-white px-4 py-3"><option value="">انتخاب برند</option>{vehicleBrands.map((item) => <option key={item.id} value={item.id}>{item.name_fa}</option>)}</select></label>
