...
export default function MyDealershipClient({ initialData }: { initialData: MyDealershipPageData }) {
  ...
  return <MobileAppShell><div dir="rtl" className="mx-auto w-full max-w-xl px-4 pb-32 pt-[72px]"><header className="fixed inset-x-0 top-0 z-[100] mx-auto w-full max-w-xl border-b border-gray-100 bg-white/95 shadow-sm backdrop-blur-xl"><div className="flex h-[56px] w-full items-center justify-between gap-3 px-4"><h1 className="min-w-0 flex-1 truncate text-right text-[21px] font-black tracking-tight text-gray-950">{title}</h1><span className={`shrink-0 rounded-full px-2.5 py-1 text-xs font-extrabold ${isActive ? "bg-emerald-50 text-emerald-600" : "bg-red-50 text-red-600"}`}>{isActive ? "فعال" : "غیرفعال"}</span></div></header>
  <Link href="/vehicles/new" className="mb-5 mt-3 flex h-14 w-full items-center justify-center gap-2 rounded-[19px] bg-emerald-600 text-[15px] font-extrabold text-white shadow-[0_8px_24px_rgba(16,185,129,0.18)] active:scale-[0.99]"><span className="text-xl leading-none">＋</span>ثبت خودروی جدید</Link>
