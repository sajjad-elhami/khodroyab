create or replace function public.set_inventory_confirmation_on_insert()
returns trigger
language plpgsql
as $$
begin
  if new.status = 'available' and new.inventory_confirmed_at is null then
    new.inventory_confirmed_at := now();
  end if;
  return new;
end;
$$;

drop trigger if exists trg_vehicles_inventory_confirmation_on_insert on public.vehicles;

create trigger trg_vehicles_inventory_confirmation_on_insert
before insert on public.vehicles
for each row
execute function public.set_inventory_confirmation_on_insert();
